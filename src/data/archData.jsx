...
const archChapter = {
  ...
  overview: {
    ...
    oneLiner:
      '拿着 374 条系统需求（SR），设计出系统的「骨架」——14 个子系统怎么分、软硬件怎么分层、芯片怎么选、接口怎么定义、每条 SR 分给谁。此后向下游分叉：硬件支（HWE.1）与软件支（SWE.1）并行开展。',
    analogy:
      '把 sys-arch 想象成「画房屋结构图」：sys-analysis 给了你需求清单（几房几厅、要通水通电），sys-arch 就是建筑师把需求落成图纸——墙怎么砌（子系统划分）、水电怎么走（接口设计）、用什么样的材料（芯片选型）、每一间房住什么（SR 分配到子系统）。图纸定稿后，水电工（硬件 HWE.1）和木工（软件 SWE.1）就按同一张图各自并行施工。',
