...
export const graphNodes = [
  // 阶段一（cyan）
  { id: 'init', cmd: 'init', label: 'SOR 解析', stage: 'ACQ.4', out: '解析产物（清单/副本/歧义）', real: '', color: 'phase1', icon: '📄', desc: '阶段一第 1 节点：把原始需求文档变成「干净 + 已知疑点」的结构化输入。本课程当前章节。' },
  { id: 'clarify', cmd: 'clarify', label: '需求澄清', stage: '—', out: 'CLQ 澄清矩阵', real: '', color: 'phase1', icon: '❓', desc: '读 init 找出的 45 条歧义，逐条向客户问清楚，生成澄清矩阵。' },
  { id: 'prd-analysis', cmd: 'prd-analysis', label: '产品需求', stage: 'SYS.1', out: 'PRD 文档', real: '', color: 'phase1', icon: '📝', desc: '基于结构化输入，产出产品需求文档（PRD）。' },
  { id: 'sys-analysis', cmd: 'sys-analysis', label: '系统需求', stage: 'SYS.2', out: '系统需求（374 条）', real: '', color: 'phase1', icon: '🧩', desc: '把 PRD 的 373 条产品需求转成 374 条系统需求。' },
  { id: 'sys-arch', cmd: 'sys-arch', label: '系统架构', stage: 'SYS.3', out: '系统架构（14 子系统）', real: '', color: 'phase1', icon: '🏗️', desc: '把系统需求设计成架构：14 个子系统、4 层软件。' },
  { id: 'hwe-analysis', cmd: 'hwe-analysis', label: '硬件分析', stage: 'HWE.1', out: '硬件需求分析', real: '', color: 'phase1', icon: '🔧', desc: '分析硬件相关需求（与系统架构并行，供软件需求参考）。' },
  { id: 'swe-analysis', cmd: 'swe-analysis', label: '软件需求', stage: 'SWE.1', out: '软件需求（294 条）', real: '', color: 'phase1', icon: '🧠', desc: '把系统需求细化成软件需求，共 294 条。' },
  { id: 'swe-arch', cmd: 'swe-arch', label: '软件架构', stage: 'SWE.2', out: '软件架构（57 模块）', real: '', color: 'phase1', icon: '🗺️', desc: '把软件需求组织成模块：57 个软件模块。' },
  { id: 'swe-arch-if', cmd: 'swe-arch-if', label: '接口契约', stage: 'SWE.3', out: '接口契约（19 份）', real: '', color: 'phase1', icon: '🔌', desc: '定义模块间怎么调用：19 份接口契约 + 注册表。' },
  { id: 'swe-coding-plan', cmd: 'swe-coding-plan', label: '编码计划', stage: 'SWE.4', out: '编码计划（19 模块）', real: '', color: 'phase1', icon: '📋', desc: '为每个模块制定编码计划：做什么、怎么验证。' },
  { id: 'swe-coding-do', cmd: 'swe-coding-do', label: '编码执行', stage: 'SWE.4', out: 'MQTT 源码（19 模块）', real: '', color: 'phase1', icon: '💻', desc: '按计划写代码，产出 MQTT 功能源码。' },
  { id: 'swe-coding-verify-pc', cmd: 'swe-coding-verify-pc', label: 'PC 验证', stage: 'SWE.4', out: 'PC 验证报告（25 用例全过）', real: '', color: 'phase1', icon: '🖥️', desc: '在 PC 上验证 MQTT 功能 —— 阶段一的完成标志，25 个用例全通过。' },
  // 阶段二（green）
  { id: 'sqt-strategy', cmd: 'sqt-strategy', label: '测试策略', stage: 'SYS.5', out: '测试策略方案', real: '', color: 'phase2', icon: '🎯', desc: '制定测试策略：测什么、怎么测、分几类（功能/性能/接口）。' },
  { id: 'sqt-tr-analysis', cmd: 'sqt-tr-analysis', label: '测试需求', stage: 'SYS.5', out: '测试需求（16 项）', real: '', color: 'phase2', icon: '🔎', desc: '从需求推导出要测什么：16 项测试需求。' },
  { id: 'sqt-case-design', cmd: 'sqt-case-design', label: '用例设计', stage: 'SYS.5', out: '测试用例', real: '', color: 'phase2', icon: '🧪', desc: '把测试需求写成一条条可执行的用例。' },
  { id: 'sqt-script-gen', cmd: 'sqt-script-gen', label: '脚本生成', stage: 'SYS.5', out: '自动化测试脚本', real: '', color: 'phase2', icon: '🤖', desc: '把用例变成能自动跑的脚本。' },
  { id: 'sqt-auto-test', cmd: 'sqt-auto-test', label: '自动化测试', stage: 'SYS.5', out: '测试结果（真机 5 轮）', real: '', color: 'phase2', icon: '⚙️', desc: '在真机上自动跑测试，迭代 5 轮修复问题。' },
  { id: 'sqt-defect-feedback', cmd: 'sqt-defect-feedback', label: '缺陷闭环', stage: 'SUP.8', out: '缺陷报告 + 反馈', real: '', color: 'phase2', icon: '📋', desc: '把测试发现的缺陷登记、反馈给上游修复、验证后关闭。' },
  // 验证支撑（blue）
  { id: 'swe-static-verify', cmd: 'swe-static-verify', label: '静态分析', stage: 'SUP.1', out: '静态分析报告', real: '', color: 'support', icon: '🧹', desc: '用工具扫代码找潜在问题（不改代码，纯检查）。' },
  { id: 'swe-unit-verify', cmd: 'swe-unit-verify', label: '单元验证', stage: 'SWE.4', out: '单元测试报告', real: '', color: 'support', icon: '🧩', desc: '每个函数单独测：16 个模块 56 个用例。' },
  { id: 'swe-integration-verify', cmd: 'swe-integration-verify', label: '集成验证', stage: 'SWE.5', out: '集成测试报告', real: '', color: 'support', icon: '🔗', desc: '模块拼起来一起测：11 组 56 个用例。' },
  { id: 'swe-coding-verify-v2', cmd: 'swe-coding-verify-v2', label: '设备级验证', stage: 'SWE.4', out: '真机验证报告', real: '', color: 'support', icon: '📡', desc: '烧到真机上验证：能启动、业务模块全活、持续上报。' },
  // 治理环节（amber）
  { id: 'change', cmd: 'yxspec:change', label: '变更管理', stage: 'SUP.10', out: '变更记录', real: '', color: 'govern', icon: '🔁', desc: '文档有更新时记录变更，决定哪些下游要重跑。' },
  { id: 'feedback', cmd: 'yxspec:feedback', label: '上游反馈', stage: 'SUP.8', out: '反馈闭环', real: '', color: 'govern', icon: '📬', desc: '发现上游文档/流程的问题，登记反馈，跟进到关闭。' },
  { id: 'review', cmd: 'yxspec:review', label: '阶段审查', stage: 'SUP.1', out: '审查报告 + 签核', real: '', color: 'govern', icon: '🛡️', desc: '每个阶段产物要过审查（技术 + 质量双签）才能进下一步。' },
];

export const graphEdges = [
  // 主链（实线 = 实际执行）
  { from: 'init', to: 'clarify', label: 'amb_index.json (45 AMB)' },
  { from: 'init', to: 'prd-analysis', label: '结构化输入' },
  { from: 'clarify', to: 'prd-analysis', label: 'CLQ 澄清矩阵' },
  { from: 'prd-analysis', to: 'sys-analysis', label: 'PRD 文档' },
  { from: 'sys-analysis', to: 'sys-arch', label: 'sys-req (374 SR)' },
  { from: 'sys-analysis', to: 'hwe-analysis', label: '硬件需求抽取' },
  { from: 'sys-arch', to: 'swe-analysis', label: '系统架构' },
  { from: 'hwe-analysis', to: 'swe-analysis', label: 'hw-analyse', dashed: true },
  { from: 'swe-analysis', to: 'swe-arch', label: 'SW-SRS (294 SWR)' },
  ...
  { from: 'swe-coding-do', to: 'swe-coding-verify-pc', label: 'MQTT 源码' },
  ...
  { from: 'swe-coding-verify-pc', to: 'swe-coding-verify-v2', label: '承接设备级' },
  ...
  // SQT 链
  { from: 'sqt-strategy', to: 'sqt-tr-analysis', label: 'SQT-TP' },
  { from: 'sqt-tr-analysis', to: 'sqt-case-design', label: '16 TR' },
  { from: 'sqt-case-design', to: 'sqt-script-gen', label: '全域 TC' },
  { from: 'sqt-script-gen', to: 'sqt-auto-test', label: 'Behave 脚本' },
  { from: 'sqt-auto-test', to: 'sqt-defect-feedback', label: '测试结果 (88/116)' },
  // 治理（虚线 = 支撑关系）
...
export const graphLegend = [
  { color: 'phase1', label: '阶段一主链（11 节点）' },
  { color: 'phase2', label: '阶段二 SQT（6 节点）' },
  { color: 'support', label: '验证支撑（4 节点）' },
  { color: 'govern', label: '治理环节（3 节点）' },
];
...
export const stages = [
  ...
  { key: 'sys-arch', label: 'sys-arch', name: '系统架构', stage: 'SYS.3', available: true, desc: '系统架构设计（14 子系统）' },
  { key: 'hwe-analysis', label: 'hwe-analysis', name: '硬件分析', stage: 'HWE.1', available: true, desc: '硬件需求分析（hw-analyse）' },
  { key: 'swe-analysis', label: 'swe-analysis', name: '软件需求', stage: 'SWE.1', available: true, desc: '软件需求分析（294 SWR）' },
  ...
  { key: 'swe-coding-verify-pc', label: 'verify-pc', name: 'PC 验证', stage: 'SWE.4', available: true, desc: 'MQTT 功能 PC 端验证' },
  // 阶段二（6）
  ...
  // 验证支撑（4）
  { key: 'swe-static-verify', label: 'static-verify', name: '静态分析', stage: 'SUP.1', available: true, desc: 'Cppcheck+MISRA' },
  { key: 'swe-unit-verify', label: 'unit-verify', name: '单元验证', stage: 'SWE.4', available: true, desc: '16 模块 56 用例' },
  { key: 'swe-integration-verify', label: 'integration-verify', name: '集成验证', stage: 'SWE.5', available: true, desc: '11 组 56 用例' },
  { key: 'swe-coding-verify-v2', label: 'verify-v2', name: '设备级验证', stage: 'SWE.4', available: true, desc: '真机启动验证' },
  // 治理环节（3）
  { key: 'change', label: 'change', name: '变更管理', stage: 'SUP.10', available: true, desc: '变更记录' },
  { key: 'feedback', label: 'feedback', name: '上游反馈', stage: 'SUP.8', available: true, desc: 'UF 闭环' },
  { key: 'review', label: 'review', name: '阶段审查', stage: 'SUP.1', available: true, desc: 'review + 双签' },
];
